...
api_token = pd.read_csv(os.path.join(project_dir, '.openFDA.params')).api_key.values[0]
...
out = os.path.join(data_dir, "openFDA_drug_event")
out_report = os.path.join(out, "report") + os.sep
out_meta = os.path.join(out, "meta") + os.sep
out_patient = os.path.join(out, "patient") + os.sep
out_patient_drug = os.path.join(out, "patient_drug") + os.sep
out_patient_drug_openfda = os.path.join(out, "patient_drug_openfda") + os.sep
out_patient_drug_openfda_rxcui = os.path.join(out, "patient_drug_openfda_rxcui") + os.sep
out_patient_reaction = os.path.join(out, "patient_reaction") + os.sep

for directory in [
    data_dir,
    out,
    out_report.rstrip(os.sep),
    out_meta.rstrip(os.sep),
    out_patient.rstrip(os.sep),
    out_patient_drug.rstrip(os.sep),
    out_patient_drug_openfda.rstrip(os.sep),
    out_patient_drug_openfda_rxcui.rstrip(os.sep),
    out_patient_reaction.rstrip(os.sep)
]:
...
with open(filehandle, 'r', encoding='utf-8') as stream:
    try:
